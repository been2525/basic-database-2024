...
- MS SQL Server 설치 : https://www.microsoft.com/ko-kr/sql-server/sql-server-downloads 최신버전
    - DBMS 엔진 - 개발자버전
        ...
        ![기능선택](https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db001.png)
...
- SQL 기본학습
    ...
    ![SSMS로그인](https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db002.png)
...
- Datavbase 학습
    ...
    - ServerName(HostName) - [자기컴퓨터이름|내네트워크주소|127.0.0.1(LoopBack IP)|localhost(LoopBack URL)] 중에서 선호하는거 아무거나
    - 관계 데이터 모델
        - 릴레이션 - 행과 열로 구성된 테이블
...
- DML 학습
    -  SELECT문
        ...
        ![외부조인](https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db004.png)
    ...
    - 관계 데이터 모델
        - 무결성 제약조건
            ...
            - 외래키(Foreign Key!) - 기본키를 참조하여 사용하는 것
                - 고려사항 - 다른 릴레이션과의 관계, 다른 릴레이션의 기본키를 호칭, 서로 같은 값이 사용, 기본키가 변경되면 외래키도 변경되어야함.
                            NULL과 중복을 허용(Not NULL인 경우도 있음), 자기자신의 기본키르 외래키로 사용할 수도 있음, 외래키가 기본키의 속성중 하나가 될 수도 있음

            - 데이터 무결성(Intergrity) - DB에 저장된 데이터의 일관성과 정확성을 지키는 것
                - 도메인 무결성 제약조건 - 데이터 타입, Null, 기본값, 체크 특성을 지키는 것
                - 개체 무결성 제약조건 - 기본키 제약조건으로 부름. Unique에 NOT NULL.(값이 중복되어도 안되고, 빠져도 안됨)
                - 참조 무결성 제약조건 - 외래키 제약조건으로 부름. 부모의 키가 아닌값은 사용할 수 없음. 외래키가 바뀔때 기본키의 값이 아닌 것은 제약을 받는다.
...
- DML 학습
    - SELECT문
        - OUTER JOIN(외부조인)
            ...
            - 왼쪽테이블을 기준으로 조건에 일치하지 않는 왼쪽테이블 데이터도 모두 표시(LEFT OUTER JOIN)
            - 오른쪽 테이블 기준으로 조건에 일치하지않는 오른쪽테이블 데이터 모두 표시(RIGHT OUTER JOIN)
        - 부속질의(SubQuery)
            - 쿼리 내에 다시 쿼리를 작성하는 것
            - 서브쿼리를 쓸 수 있는 장소
                - SELECT절 - 한컬럼에 하나의 값만
        ...
        - 집합연산 - JOIN도 집합이지만, 속성별로 가로로 병합하기 때문에 집합이라 부르지 않음. 집합은 데이터를 세로로 합치는 것을 뜻함
            ...
            - 차집합(UNION, 진짜 많이 사용함) - UNION(중복제거), UNION ALL(중복허용)
            - 교집합(INTERSECT, 거의 사용안함) - 두 테이블에 모두 존재하는 값
            - EXISTS - 데이터 자체의 존재여부로

- DDL 학습 -- SSMS에서 마우스로 조작이 편리
    - CREATE - 개체(데이테베이스, 테이블, 뷰, 사용자 등)를 생성하는 구문
    ```sql
    -- 테이블 생성에 한정
    CREATE TABLE 테이블명
    ({속성이름 데이터 타입
    ...
    - DROP - 개체를 삭제하는 구문
        ...
        - 외래키로 사용되는 기본키가 있으면, 외래키를 사용하는 테이블을 삭제 후, 기본키의 테이블 삭제해야함
...
- 파이썬 SQL Server연동 프로그래밍
    ...
    - SQL Server 데이터 핸들링
        ...
        ![구성관리자](https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db005.png)
...
- 파이썬 SQL Server연동 프로그래밍
    ...
    - 문제점 - 한글 깨짐문제
        1. DB 테이블의 varchar(ASCII) -> nvarchar(UTF-8)
    ...
    - 실행화면
        ...
        https://github.com/been2525/basic-database-2024/assets/130003854/6dd078b3-1c69-4525-ba72-9870aed48e66
...
- SQL 고급
    - 트랜젝션 - All or Nothing
    - 트랜젝션 속성 - A(tomicty),C(onsistency),  I(olation), D(urablity)
    - TCL에서 사용할 키워드
        - BEGIN, TRAN[SACTION], COUNT, ROLLBACK, SAVE

    - SQL Server는 기본적으로 Auto Commit(시스템이 자동으로 트랜잭션을 건다)
    - SSMS > 도구 > 옵션 > 쿼리 실행 > SQL Server > ANS -> SET IMPLICIT_TRANSACTIONS 체크 프로잭트 재시작
    
    - 트랜잭션 - 로직 처리시 다른 트랙잭션의 간섭을 받지 않기 위한 것(Lock), 중요한 데이터 수정, 삭제시 잘못된 변경을 방지하기 위한 것

- 데이터베이스 모델링
    - 설계 순서 -> 개념 설계 -> 논리 설계 -> 물리 설계
    - 개념 모델링 - 요구사항을 받으면서 정해지지 않은 여러 개채들을 정립화 하는 단계
    - 논리 모델링 - 기본키 지정, 외래키 지정, 관계 정립, 속성들 이림(한글) 개체를 정하는 단계
    ...
    <!--md 주석. ![인덱스](https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db006.png) -->
    <!-- html img 태그가 이미지 사이즈 조정 가능 -->
    <img src="https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db006.png" width="870">
...
- 정규화
    ...
    - 보통 BCNF까지 정규화를 한다.
    - 4정규형(다치 종속성), 5정규형(조인 종속성, 무손실 분해)은 잘 안한다.
...
- 실무실습(사용자, 권한...)
    1. DB관리자(SSMS)
        ...
        - hrDB를 사용할 사용자 계정을 생성, 필요한 권한 설정
            ...
            - SSMS > 보안 > 로그인 > sa 존재 > 속성에서 비밀번호를 변경 이후 다시 SQL Server 인증으로 sa 비밀번호 입력후 로그인 하면 됨
...
- 실무실습
    ...
    ![HR_ERD](https://raw.githubusercontent.com/been2525/basic-database-2024/main/images/db007.png)
